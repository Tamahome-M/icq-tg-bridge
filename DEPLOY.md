...
`bridge.db` вместе с копиями, каталоги `photos`, `render`, `downloads` и
`claude`, файлы журналов и всё, что начинается с точки (`.venv`, `.claude`
со входом в Claude Code — каталог установки служит пользователю моста
домом).
