...
Не трогает вовсе, ни перезаписью, ни удалением: `config.toml`, файлы сессий
(`tg.session`, `max.session`), `bridge.db` вместе с копиями, каталоги
`photos`, `render`, `downloads` и `claude`, файлы журналов и всё, что
начинается с точки (`.venv`, `.claude` со входом в Claude Code — каталог
установки служит пользователю моста домом). Устаревшие файлы удаляются
только внутри каталогов, которые есть в самом архиве (`bridge/`, `tests/`,
`tools/`, `contrib/`); корень установки и чужие каталоги не чистятся никогда,
а `-n` показывает список «удалится» заранее.
