...
ATTACH_FILE = 0x04           # документ: «Скачать файл» — на карту телефона
# Типы приметы в запросе к службе 0x10 сверх аватарки (0x0001):
# снимок по токену и история чата (в «хеше» — число сообщений).
BART_PHOTO = 0x0080
BART_HISTORY = 0x0081
BART_VIDEO = 0x0082          # сам ролик: приходит частями, по части в 10/07
BART_VOICE = 0x0083          # голосовое: AMR в 3GP, тоже частями
BART_CHATS = 0x0084          # весь список чатов: и те, что не влезли в контакт-лист
BART_OPEN = 0x0085           # вернуть чат на телефон (в «хеше» — его UIN)
BART_FILE = 0x0086           # документ по токену: частями, в первой части — имя файла

# Как называть приметы в журнале: «телефон просит ролик для …».
BART_NAMES = {
    BART_PHOTO: "снимок",
    BART_HISTORY: "историю",
    BART_VIDEO: "ролик",
    BART_VOICE: "голосовое",
    BART_CHATS: "список чатов",
    BART_OPEN: "вернуть чат",
    BART_FILE: "файл",
}
# Снимок с камеры телефона в обратную сторону: клиент шлёт его частями
# 10/02, мост отвечает 10/03 — принял или нет.
SSBI_UPLOAD = 0x0002
SSBI_UPLOAD_ACK = 0x0003
SSBI_UPLOAD_VOICE = 0x0004   # то же, но голосовое: перед куском — длительность
SSBI_UPLOAD_VIDEO = 0x0005   # и «кружок»: та же раскладка, что у голосового
SSBI_UPLOAD_FILE = 0x0008    # файл с телефона: части с общим размером, в первой — имя (0x0006/0x0007 — запрос и ответ аватарки)
UPLOAD_MAX_BYTES = 4 * 1024 * 1024   # предохранитель: больше не собираем (кружок с V8 — сотни КБ)
VIDEO_PART_BYTES = 60_000    # размер части; FLAP держит до 64 КБ. Короткий ролик
                             # укладывается в один пакет — тот же путь, что у фото.
HISTORY_MAX_BYTES = 12 * 1024    # столько текста истории отдаём за раз
CHATS_MAX_BYTES = 24 * 1024      # столько списка чатов — записи по ~40 байт
